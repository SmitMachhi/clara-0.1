...
**Step 1:** Converted all backfill functions in `schema-backfills.ts` to use `.iterate()` instead of `.all()`. Updated `migrateEncryptedDataToNewKey`, `backfillTemplateParsedJson`, `backfillTemplatePresetParsedJson`, `backfillLocationsEncryptedData`, `backfillEntryCapturedCoordinates`, and `backfillEntryLocationIdEncryption` to stream rows instead of loading full tables into memory. Added type annotations for the iterable rows, used a boolean flag to track if any rows were processed for short-circuiting, and added comments explaining the switch to iteration to avoid peak memory usage. Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**.

**Step 2:** Converted the export endpoint to stream JSON instead of building the entire dataset in memory. Added `iterateEntriesWithRawData()` function in `entries.ts` that uses `prepare(...).iterate()` to iterate over entries with raw encrypted data. Updated `+server.ts` to use `ReadableStream` and emit JSON in chunks: static fields first, then entries one at a time with proper comma handling, decrypting and parsing each entry on the fly. Used `TextEncoder` to encode strings before enqueueing. Preserved exact response format including all property names and null handling for failed decrypts. Built location name lookup map once outside the stream to avoid repeated queries. Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**.

**Step 3:** Converted backup creation and download to use streaming instead of loading entire DB into memory. Updated `createBackup()` in `backups.ts` to async function that uses `fs.createReadStream()` and `crypto.createCipheriv` with `stream/promises pipeline()` to stream encryption to a temp file, then reconstructs final file with correct format (IV + authTag + ciphertext) to match original format. Updated `decryptBackup()` to return a `Readable` stream that reads IV (bytes 0-11) and authTag (bytes 12-27) using separate streams with Promises, then pipes ciphertext (bytes 28+) through `createDecipheriv`. Used a PassThrough stream to coordinate async header reading with decryption streaming. Updated `+server.ts` to handle async `createBackup()` and return streaming response using `Readable.toWeb()` for decrypted backups. Kept Content-Type and Content-Disposition headers, removed Content-Length since streaming. All changes maintain O(1) memory and preserve original encrypted file format. Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**.

**Step 4:** Limited entry dates to a year scope to reduce memory and response size. Added `getEntryDatesForYear(year: number)` function in `entries.ts` that queries entries within the specified year using `WHERE date >= 'YYYY-01-01' AND date <= 'YYYY-12-31'`. Updated `GET` handler in `+server.ts` to accept optional `year` query param, defaulting to current year if not provided or invalid. Updated `fetchEntries()` in `journal-actions.ts` to accept optional `year` argument and include it in the query string using URLSearchParams. Updated `loadJournalPageData()` in `journal-page-helpers.ts` to pass the current year to `fetchEntries`. All changes maintain existing response shape for `recentEntries` and ensure journal page still uses year-based dates from current year for stats. Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**.

**Step 5:** Converted passphrase verification from blocking sync to non-blocking async to prevent event loop blocking during authentication on shared CPU. Updated imports in `auth.ts` to use async `pbkdf2` instead of `pbkdf2Sync` and created `pbkdf2Async` helper using `util.promisify`. Changed `verifyPassphrase()` to async function returning `Promise<boolean>` and updated both call sites in `src/routes/api/auth/+server.ts` (line 60) and `src/routes/api/wipe/+server.ts` (line 30) to await the result. Preserved all error handling, rate limiting, and PBKDF2 settings (100,000 iterations, 32 byte key, sha256). Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**.

**Step 6:** Completed verification and final QA. Ran `npm run build` which completed successfully with no errors, generating both client and server bundles. Started dev server with `npm run dev` and confirmed the application loads correctly with HTML response from localhost:5173. Verified health endpoint responds (returns unauthorized as expected). All 6 optimization steps are now complete: backfills use streaming iteration, export endpoint streams JSON, backup creation/download streams, entry dates are limited to current year scope, and passphrase verification is non-blocking async. **Bonus fix:** Fixed legacy unencrypted backup download which was loading entire file into memory using `readFileSync()`. Updated `+server.ts` to use `fs.createReadStream()` and `Readable.toWeb()` for unencrypted backups, maintaining O(1) memory for both encrypted and legacy backup downloads. Removed unused `readFileSync` import. Ran `npx svelte-check --threshold error` with result: **0 errors and 0 warnings**. The app should now be stable on 256MB Fly.io VM with significantly reduced peak memory usage and CPU spikes.
